...
export async function handleGetPushConfig(cookieHeader) {
  ...
  const [usernames, pushUsers] = await Promise.all([listUsernames(), listAllPushConfigs()])
  const byName = new Map(pushUsers.map((u) => [String(u.username).toLowerCase(), u]))

  const users = []
  for (const username of usernames) {
    const hit = byName.get(username.toLowerCase())
    byName.delete(username.toLowerCase())
    if (hit) {
      users.push({ username, ...publicPushConfig(hit.config) })
      continue
    }
    // 索引缺失时直接读该用户配置，避免后台显示与定时任务不一致
    const config = await getUserPushConfig(username)
    users.push({ username, ...publicPushConfig(config) })
  }

  for (const leftover of byName.values()) {
    users.push({
      username: leftover.username,
      ...publicPushConfig(leftover.config),
    })
  }

  users.sort((a, b) => a.username.localeCompare(b.username, 'zh'))
  return { status: 200, data: { ok: true, users } }
}
