...
export function isAdminUsername(username, env = process.env) {
  const { user } = getAuthConfig(env)
  if (!user || !username) return false
  return String(username).toLowerCase() === String(user).toLowerCase()
}

export function signSession(username, secret, { role = 'user', maxAgeSec = MAX_AGE_SEC } = {}) {
  const payload = {
    u: username,
