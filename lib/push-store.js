...
/** 推送用户 Hash：field=用户名小写，value=配置 JSON */
const USERS_HASH_KEY = 'sv:push:users'
...
async function readUsersMap(redis) {
  // 1) 优先读 Hash
  try {
    const hash = await redis.hgetall(USERS_HASH_KEY)
...
async function writeUsersMap(redis, map) {
  const payload = {}
  for (const [key, cfg] of Object.entries(map)) {
    payload[String(key).toLowerCase()] = JSON.stringify({ ...cfg, username: cfg.username || key })
  }
  try {
    await redis.del(USERS_HASH_KEY)
  } catch {
    // ignore
  }
  if (Object.keys(payload).length) {
    await redis.hset(USERS_HASH_KEY, payload)
  }
}
