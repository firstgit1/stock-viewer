...
/** 所有推送用户配置（单 key JSON，避免 Hash del/hset 竞态清空） */
const USERS_JSON_KEY = 'sv:push:users-json'
...
async function readUsersMap(redis) {
  // 权威来源
  try {
    const raw = await redis.get(USERS_JSON_KEY)
    if (raw) {
      const parsed =
        typeof raw === 'string'
          ? (() => {
  ...
  // 兼容旧 Hash
  try {
    const hash = await redis.hgetall('sv:push:users')
    if (hash && typeof hash === 'object' && Object.keys(hash).length) {
      const map = {}
      for (const [field, value] of Object.entries(hash)) {
        ...
        map[String(field).toLowerCase()] = cfg
      }
      if (Object.keys(map).length) {
        await redis.set(USERS_JSON_KEY, map)
        return map
      }
    }
  } catch {
  ...
  // 兼容更旧的 map key
  try {
    const raw = await redis.get('sv:config:push-users-map')
    if (raw) {
      const parsed =
        typeof raw === 'string'
          ? (() => {
          ...
          : raw
      if (parsed && typeof parsed === 'object' && !Array.isArray(parsed)) {
        await redis.set(USERS_JSON_KEY, parsed)
        return { ...parsed }
      }
    }
  } catch {
...
/** 排查用：不返回 Token 明文 */
export async function getPushStorageDiagnostics() {
  ...
  for (const [key, cfg] of Object.entries(map)) {
    const perKey = await redis.get(userKey(key))
    let keyHasToken = false
    if (perKey) {
      const parsed =
        typeof perKey === 'string'
          ? (() => {
              try {
                return JSON.parse(perKey)
              } catch {
                return null
              }
            })()
          : perKey
      keyHasToken = Boolean(parsed?.token)
    }
    entries.push({
      user: key,
      mapEnabled: Boolean(cfg?.enabled),
      mapHasToken: Boolean(cfg?.token),
      mapUpdatedAt: cfg?.updatedAt || null,
      keyHasToken,
    })
  }
  ...
  // 额外抽查常见用户名的单 key
  const probes = {}
  for (const name of ['admin', 'fxk']) {
    const perKey = await redis.get(userKey(name))
    probes[name] = {
      keyExists: Boolean(perKey),
      keyHasToken: Boolean(
        perKey &&
          (typeof perKey === 'object'
            ? perKey.token
            : (() => {
                try {
                  return JSON.parse(perKey)?.token
                } catch {
                  return ''
                }
              })()),
...
async function writeUsersMap(redis, map) {
  // 显式 JSON 字符串写入，避免 SDK 序列化差异导致读回为空
  await redis.set(USERS_JSON_KEY, JSON.stringify(map))
}
...
export async function saveUserPushConfig(username, patch = {}) {
  ...
  // 仅更新推送结果：直接改已有记录字段，避免读空配置把 mode/token 打回默认
  if (metaOnly) {
    if (redis) {
      const map = await readUsersMap(redis)
      const existing = map[lower]
      if (!existing || (!existing.token && !existing.enabled)) {
        return current
      }
      const merged = normalizeConfig({
        ...normalizeConfig(existing),
        lastRunAt: patch.lastRunAt !== undefined ? patch.lastRunAt : existing.lastRunAt,
        lastResult: patch.lastResult !== undefined ? patch.lastResult : existing.lastResult,
        updatedAt: existing.updatedAt || current.updatedAt,
      })
      map[lower] = { ...merged, username: existing.username || name }
      await writeUsersMap(redis, map)
      await redis.set(userKey(name), JSON.stringify(merged))
      return merged
    }
    if (!isUserStoreConfigured()) return current
    const users = await readLocalUsers()
    const existing = users[lower]
    if (!existing || (!existing.token && !existing.enabled)) return current
    const merged = normalizeConfig({
      ...normalizeConfig(existing),
      lastRunAt: patch.lastRunAt !== undefined ? patch.lastRunAt : existing.lastRunAt,
      lastResult: patch.lastResult !== undefined ? patch.lastResult : existing.lastResult,
    ...
    users[lower] = { ...merged, username: existing.username || name }
  ...
  if (next.enabled && !next.token) {
    const err = new Error('开启推送前请先填写 PushPlus Token')
    err.code = 'INVALID'
    throw err
  }
  ...
  if (redis) {
    ...
    await writeUsersMap(redis, map)
    // 双写单用户 key（显式 JSON），便于排查与兼容
    await redis.set(userKey(name), JSON.stringify(next))

    const verifyMap = await readUsersMap(redis)
    const verified = normalizeConfig(verifyMap[lower] || {})
    if (next.token && verified.token !== next.token) {
      const err = new Error('保存失败：写入后校验未通过，请检查 Upstash Redis 环境变量是否指向正确实例')
      err.code = 'UNAVAILABLE'
      throw err
    }
    if (next.enabled && !verified.token) {
      const err = new Error('保存失败：Redis 中未读到 Token')
      err.code = 'UNAVAILABLE'
      throw err
    }
    if (next.mode && verified.mode !== next.mode) {
      const err = new Error('保存失败：推送规则未写入成功，请重试')
      err.code = 'UNAVAILABLE'
      throw err
    }
    return next
